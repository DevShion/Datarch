...
export default class Collection<T extends { [key in string]: any }> {
  ...
  static Schemas = {
    auth: Functions.Zod.TypedSchema<
      Types.Request.Auth["auth"]
    >()(z.strictObject({
      id: z.string(),
      password: z.string(),
    })),
    convex: Functions.Zod.TypedSchema<
    ...
    // deno-lint-ignore no-explicit-any
    findQuery: Functions.Zod.TypedSchema<Types.Method.FindType<any>["Req"]["query"]>()(
      z.strictObject({
  ...
  constructor(args: {
    ...
    this.Find = new Method({
      reqSchema: Functions.Zod.TypedSchema<
    ...
    this.Create = new Method({
      reqSchema: Functions.Zod.TypedSchema<Omit<Types.Method.CreateType<T>["Req"], "data">>()(z.strictObject({
        auth: Collection.Schemas.auth,
